...
export const resellerService = {
  ...
  async getResellers(agencyId: string): Promise<Reseller[]> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Failed to get resellers:', error);
      return [];
    }
  },
  ...
  async updateResellerStatus(agencyId: string, id: string, status: 'ACTIVE' | 'SUSPENDED'): Promise<void> {
    if (import.meta.env.DEV) console.info(`Status update requested for reseller ${id}: ${status}`);
  },
  ...
  async deleteReseller(agencyId: string, id: string): Promise<void> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Failed to delete reseller:', error);
    }
  }
};
