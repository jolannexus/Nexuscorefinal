...
export const supplierService = {
  ...
  async getConnections(agencyId: string): Promise<SupplierConnection[]> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Failed to get supplier connections:', error);
      return [];
    }
  },
  ...
  async updateConnection(agencyId: string, id: string, data: Partial<SupplierConnection>): Promise<void> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Failed to update connection:', error);
    }
  },
  ...
  async deleteConnection(agencyId: string, id: string): Promise<void> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Failed to delete connection:', error);
    }
  },
  ...
  async syncConnection(agencyId: string, id: string): Promise<void> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Balance sync failed:', error);
      throw error;
    }
  },
  ...
  async syncProducts(agencyId: string, id: string): Promise<{ count: number; total: number }> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Product sync failed:', error);
      throw error;
    }
  }
};
